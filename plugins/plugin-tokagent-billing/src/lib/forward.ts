/**
 * Forwarder helpers shared by every route file (v2.0.0).
 *
 * Each route's body is ~3 lines: extract headers, call the typed gateway
 * method, hand the result to `send(res, upstream)`. The plugin never inspects
 * the body — the gateway is canonical.
 */
 ...
 *
 * The CLI never decodes Authorization or x-api-key — it just relays them.
 */
...
export function send(res: RouteResponse, upstream: ProxyResponse): void {
  ...
  if (upstream.body === null) {
    ...
    res.json(upstream.body as unknown);
  }
}

/**
 * Guard that returns true when the plugin is initialized and BILLING_ENABLED.
 * Writes a 503 to the response and returns false otherwise.
 */
export function ensureEnabled(res: RouteResponse): boolean {
