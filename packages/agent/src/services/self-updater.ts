...
import type { ReleaseChannel } from "../config/types.tokagent.js";
import { CHANNEL_DIST_TAGS } from "./update-checker.js";

const NPM_PACKAGE_NAME = "tokagentos";
...
export function detectInstallMethod(): InstallMethod {
  const tokagentBin = whichSync("tokagent");
  ...
  let resolved: string;
  try {
    resolved = fs.realpathSync(tokagentBin);
  } catch {
    resolved = tokagentBin;
  }

  if (resolved.includes("/Cellar/") || resolved.includes("/homebrew/"))
    return "homebrew";
  if (resolved.includes("/snap/")) return "snap";
  if (resolved.includes("/flatpak/") || resolved.includes("ai.tokagent.Tokagent"))
    return "flatpak";
...
export function buildUpdateCommand(
  ...
  switch (method) {
    ...
    case "homebrew":
      return { command: "brew", args: ["upgrade", "tokagent"] };
    case "snap": {
      ...
      return {
        command: "sudo",
        args: ["snap", "refresh", "tokagent", `--channel=${snapCh}`],
      };
    }
    case "apt":
      return {
        ...
        args: [
          "-c",
          "sudo apt-get update && sudo apt-get install --only-upgrade -y tokagent",
        ],
      };
    case "flatpak":
      return { command: "flatpak", args: ["update", "ai.tokagent.Tokagent"] };
    case "local-dev":
      return null;
    case "unknown":
      return { command: "npm", args: ["install", "-g", spec] };
...
function readPostUpdateVersion(): string | null {
  try {
    const output = execSync("tokagent --version", {
      stdio: ["ignore", "pipe", "ignore"],
      timeout: 10_000,
    })
      .toString()
      .trim();
    // Version output may include a prefix like "tokagent/2.0.0"
