...
@pytest.fixture
def mock_tokagent_storage(temp_data_dir):
    """Create a mock TokagentOS storage adapter."""
    from tokagentos_art.tokagent_integration.storage_adapter import TokagentStorageAdapter

    return TokagentStorageAdapter(data_dir=temp_data_dir)
...
def mock_trajectory_logger(temp_data_dir):
    ...
    return TokagentTrajectoryLogger(
        agent_id="test-agent",
