...
When `OPENAI_API_KEY` is not set, the server uses `tokagentos-plugin-inmemorydb` (ephemeral) + `tokagentos-plugin-eliza-classic` so it can run deterministically without external services.
