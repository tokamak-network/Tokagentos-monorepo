//! FarcasterBot character configuration for Rust.
//!
//! Customize this to change your agent's personality, topics, and response style.

use serde_json::{json, Value};

/// Returns the FarcasterBot character configuration as JSON.
pub fn create_character() -> Value {
    json!({
        "name": "FarcasterBot",
...
mod tests {
    ...
    #[test]
    fn test_character_has_topics() {
        let c = create_character();
        assert!(!c["topics"].as_array().unwrap().is_empty());
    }
}
