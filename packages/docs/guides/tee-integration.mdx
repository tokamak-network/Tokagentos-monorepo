...
TEE integration allows your TokagentOS agents to run in secure enclaves with:
...
```bash
tokagentos create --type project --template tee my-tee-agent
cd my-tee-agent
```
...
export const character: Character = {
  name: "SecureAgent",
  plugins: [
    "@tokagentos/plugin-tee", // Add TEE plugin
  ],
  settings: {
    secrets: {
      TEE_MODE: "PRODUCTION",
...
TokagentOS supports multiple TEE providers. See the [TEE CLI Reference](/cli-reference/tee) for complete deployment commands.
...
tokagentos tee phala auth login <api-key>

# Deploy to Phala
tokagentos tee phala cvms create --name my-agent --compose ./docker-compose.yml

# Check status
tokagentos tee phala cvms list
```

### Eigen Infrastructure

```bash
tokagentos tee eigen deploy
```
