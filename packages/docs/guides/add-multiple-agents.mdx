...
```bash Terminal
tokagentos create --type agent hemingway
```
...
```json hemingway.json
{
  "plugins": [
    ...
    "@tokagentos/plugin-discord" // [!code ++]
  ]
}
```
...
```json hemingway.json
{
  "plugins": [
    ...
    "@tokagentos/plugin-elevenlabs" // [!code ++]
  ]
}
```
...
export const character: Character = {
  plugins: [
    ...
    "@tokagentos/plugin-discord",
    "@tokagentos/plugin-elevenlabs", // [!code ++]
    ...(process.env.OPENAI_API_KEY?.trim() ? ["@tokagentos/plugin-openai"] : []),
  ],
};
```
...
<CardGroup cols={2}>
  ...
  <Card title="Publish a Plugin" icon="upload" href="/guides/publish-a-plugin">
    Learn how to publish your plugins to the tokagentOS registry
  </Card>
</CardGroup>
