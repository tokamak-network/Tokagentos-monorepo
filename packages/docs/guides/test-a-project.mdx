...
description: "Write tests for your multi-agent tokagentOS project"
---

<Tip>
  **Video Tutorial**: [**Testing Projects and Plugins with
  tokagentOS**](https://www.youtube.com/watch?v=HHbY9a27b6A&list=PLrjBjP4nU8ehOgKAa0-XddHzE0KK0nNvS&index=5)
</Tip>
...
<Note>
  TokagentOS projects ship with comprehensive built-in tests for core functionality
...
describe("multi-agent configuration", () => {
  ...
  it("includes ElevenLabs plugin in both agents", () => {
    expect(shakespeare.plugins).toContain("@tokagentos/plugin-elevenlabs");
    expect(hemingway.plugins).toContain("@tokagentos/plugin-elevenlabs");
  });
...
tokagentos test --name "multi-agent"
```
