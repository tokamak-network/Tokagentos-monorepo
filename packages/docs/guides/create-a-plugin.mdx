...
<Steps>
  <Step title="Create project">
    ```bash Terminal
    tokagentos create --type project my-tokagent-project
    ```
    ...
    ```bash Terminal
    cd my-tokagent-project
    ```
  </Step>

<Step title="Create plugin inside project">
  ```bash Terminal tokagentos create --type plugin plugin-fal-ai ``` When prompted,
  choose **Quick Plugin** (we don't need frontend UI) Your structure now looks
  like: ``` my-tokagent-project/ ├── src/character.ts # Default Tokagent character └──
  ...
  <Step title="Add plugin to character">
    In `my-tokagent-project/src/character.ts`, add the local path to Tokagent's plugins array:
...
<Steps>
  <Step title="Build plugin and test connection">
    The plugin needs to be built first to create the `dist/` folder that TokagentOS loads from:
...
<Step title="Study the template structure">
  ...
  <Step title="Update index.ts to use your action">
    ...
    import { Plugin } from '@tokagentos/core';
...
<Steps>
  <Step title="Get your fal.ai API key">
    ...
    ```bash .env
    PGLITE_DATA_DIR=./.tokagent/.tokagentdb
...
<Steps>
  ...
  <Step title="Test video generation">
    Try your new action by chatting with Tokagent in the GUI (`http://localhost:3000`):
...
<Steps>
  <Step title="Add a component test">
    ...
    import type { IAgentRuntime, Memory } from '@tokagentos/core'; // [!code ++]
...
- **Better examples** - Add more conversation examples so Tokagent learns different chat patterns
...
<CardGroup cols={2}>
  <Card title="Publish a Plugin" icon="upload" href="/guides/publish-a-plugin">
    Share your plugin with the tokagentOS community
  </Card>
  <Card
  ...
  >
    Help improve tokagentOS by contributing to the core framework
  </Card>
  ...
  <Card title="CLI Reference" icon="terminal" href="/cli-reference/overview">
    Master all tokagentOS CLI commands for plugin development
  </Card>
</CardGroup>
