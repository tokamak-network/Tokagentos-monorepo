...
TokagentOS supports three response modes out of the box:
...
```typescript
import type { StreamChunkCallback } from "@tokagentos/core";
...
TokagentOS uses **stream extractors** to filter LLM output for streaming. The framework provides several built-in extractors:
...
```typescript
import { PassthroughExtractor } from "@tokagentos/core";
...
```typescript
import { XmlTagExtractor } from "@tokagentos/core";
...
```typescript
import type { IStreamExtractor } from "@tokagentos/core";
...
```typescript
import { StreamError } from "@tokagentos/core";
