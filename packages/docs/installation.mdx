---
title: "Installation"
description: "Install tokagentOS in TypeScript, Python, or Rust on macOS, Linux, or Windows"
---

tokagentOS is available in three languages with identical APIs. Choose your preferred language below.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    ```bash
    tokagentos create        # Create a new project
    cd my-project
    tokagentos start         # Start your agent
    ```

    <Tip>
      You don't need to clone the tokagentOS repository to build agents. The CLI handles everything. Only clone the monorepo if you're [contributing to core](/guides/contribute-to-core).
    </Tip>
  ...
  <Tab title="Python">
    ...
    ```bash
    pip install tokagentos
    ```

    Or with uv (faster):

    ```bash
    uv pip install tokagentos
    ```

    ### Verify Installation

    ```python
    python -c "from tokagentos import AgentRuntime; print('tokagentOS installed!')"
    ```
...
<AccordionGroup>
  ...
  <Accordion icon="python" title="Python Issues">
    ...
    **Module not found:**
    ```bash
    pip install --upgrade tokagentos
    ```
...
<CardGroup cols={2}>
  ...
  >
    Extend tokagentOS with custom functionality
  </Card>
