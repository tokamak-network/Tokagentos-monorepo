...
description: "Deploy tokagentOS agents to AWS Lambda, GCP, Vercel, Cloudflare, and Supabase"
---

Deploy AI agents as serverless functions. These examples show how to run tokagentOS on major cloud platforms with automatic scaling and pay-per-use pricing.
...
Deploy tokagentOS agents as AWS Lambda functions with API Gateway.
...
RUNTIME.get_or_init(|| {
let character = parse_character(r#"{"name": "Tokagent", "bio": "A helpful AI."}"#).unwrap();
...
supabase start
supabase functions serve tokagent-chat  # Local testing
supabase functions deploy tokagent-chat # Deploy
```
...
<CardGroup cols={2}>
  <Card title="Game Examples" icon="gamepad" href="/examples/game">
    Build AI-powered games with tokagentOS
  </Card>
