...
description: "Build HTTP endpoints for tokagentOS agents using 8 different web frameworks"
---

Create REST APIs for your tokagentOS agents using your favorite web framework. These examples work **without API keys** using the classic TOKAGENT pattern-matching algorithm.
...
<Note>
  **No API keys required!** These examples use `plugin-tokagent-classic` for
...
app.listen(PORT, () => {
console.log(`Tokagent API running on http://localhost:${PORT}`);
});
...
.unwrap_or_else(|| Uuid::new_v4().to_string());
    ...
    let content = tokagentos::types::Content {
        text: Some(req.message.clone()),
        source: Some("rest-api".to_string()),
        channel_type: Some(tokagentos::types::ChannelType::Dm),
        ..Default::default()
    };

    let mut msg =
        tokagentos::types::Memory::new(tokagentos::types::UUID::new_v4(), tokagentos::types::UUID::new_v4(), content);
...
"bio": "A Rogerian psychotherapist simulation created at MIT in 1966."
    ...
    println!("Tokagent API running on http://localhost:3000");
...
const runtime = new AgentRuntime({
character,
plugins: [sqlPlugin, openaiPlugin], // Replace tokagentClassicPlugin
});
