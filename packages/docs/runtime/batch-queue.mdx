...
description: "Why @tokagentos/core ships utils/batch-queue and how PriorityQueue, BatchProcessor, TaskDrain, and BatchQueue fit together"
---
...
- **Retries and backoff** — align with the rest of the runtime (`utils/retry` in `@tokagentos/core`).
...
- Module rationale (reviewer-oriented): [`batch-queue.ts` (core source)](https://github.com/tokagentos/tokagent/blob/main/packages/typescript/src/utils/batch-queue.ts)
- Composition and `BatchQueue` behavior: [`batch-queue/index.ts`](https://github.com/tokagentos/tokagent/blob/main/packages/typescript/src/utils/batch-queue/index.ts)
- Canonical markdown (stays in sync with this page): [`BATCH_QUEUE.md`](https://github.com/tokagentos/tokagent/blob/main/packages/typescript/docs/BATCH_QUEUE.md)
