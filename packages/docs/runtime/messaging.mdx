...
The messaging infrastructure provides real-time communication between clients and the TokagentOS server using Socket.IO. This enables instant message delivery, presence tracking, and bidirectional communication.

## Architecture

### How WebSockets Work in TokagentOS

The project uses **Socket.IO** (not raw WebSockets) for real-time communication between clients and the Tokagent server.
...
flowchart TD
    ...
    subgraph "Tokagent NextJS App"
        C[SocketIOManager]
        D[Socket.IO Client]
    end

    subgraph "Tokagent Server"
        E[Tokagent Server<br/>localhost:3000]
        F[Socket.IO Server]
    end
...
1. **Direct Connection**: Socket.IO connects directly to the Tokagent server (default: `http://localhost:3000`)
...
socket.on("connect", function () {
  console.log("[SUCCESS] Connected to Tokagent, socket ID:", socket.id);
...
function sendMessageToTokagent(text) {
...
class TokagentSocketClient {
  ...
  connect() {
    ...
    this.socket.on("connect", () => {
      console.log("Connected to Tokagent");
      this.joinRoom();
    });
...
// Usage
const client = new TokagentSocketClient("my-extension-id", "agent-room-id");
client.connect();
...
Your extension can send messages to Tokagent (server receives them), but doesn't receive responses back.
...
<Card
  title="Tokagent NextJS Starter"
  icon="github"
  href="https://github.com/tokagentOS/tokagent-nextjs-starter"
>
  A complete working example demonstrating Socket.IO integration with Tokagent,
...
For plugin developers, TokagentOS provides stream extractors to control what content is streamed from LLM responses.

```typescript
import { IStreamExtractor } from "@tokagentos/core";
