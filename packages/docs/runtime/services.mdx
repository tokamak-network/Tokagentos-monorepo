...
| ----------------- | --------------------- | ---------------------------- |
| `TRANSCRIPTION`   | Audio → Text          | `@tokagentos/plugin-openai`     |
| `VIDEO`           | Video processing      | `@tokagentos/plugin-video`      |
| `BROWSER`         | Web automation        | `@tokagentos/plugin-browser`    |
| `PDF`             | PDF processing        | `@tokagentos/plugin-pdf`        |
| `REMOTE_FILES`    | AWS S3, cloud storage | `@tokagentos/plugin-s3`         |
| `WEB_SEARCH`      | Search queries        | `@tokagentos/plugin-web-search` |
| `EMAIL`           | Email sending         | `@tokagentos/plugin-email`      |
| `TASK`            | Background tasks      | `@tokagentos/core`              |
| `WALLET`          | Crypto wallets        | `@tokagentos/plugin-evm`        |
| `MESSAGE_SERVICE` | Real-time messaging   | `@tokagentos/core`              |
...
```typescript
const ServiceType = {
  // Core services defined in @tokagentos/core
  TASK: "task",
  DATABASE: "database",
  // ... other core types
} as const;
...
const results = await searchService.search("tokagentOS");
```
...
MCP (Model Context Protocol) allows your TokagentOS agent to use external tools and services. Think of it as giving your agent abilities like web search, file access, or API connections.

### MCP Plugin Setup

```bash
bun add @tokagentos/plugin-mcp
```
...
export const character: Character = {
  ...
  plugins: [
    ...
    "@tokagentos/plugin-mcp", // Add MCP plugin
    // ... other plugins
  ],
...
export const character: Character = {
  ...
  plugins: [
    "@tokagentos/plugin-sql",
    ...(process.env.ANTHROPIC_API_KEY ? ["@tokagentos/plugin-anthropic"] : []),
    ...(process.env.OPENAI_API_KEY ? ["@tokagentos/plugin-openai"] : []),
    "@tokagentos/plugin-mcp",
  ],
  settings: {
    mcp: {
      servers: {
...
- **Tools not available**: Ensure `@tokagentos/plugin-mcp` is in your plugins array
