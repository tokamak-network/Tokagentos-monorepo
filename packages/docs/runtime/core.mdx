...
````
  ...
  <Tab title="Rust">
```rust
use tokagentos::{AgentRuntime, RuntimeOptions, parse_character};
use tokagentos_plugin_openai::create_openai_plugin;
...
const [runtime] = await tokagentOS.addAgents([{
  character,
  plugins,
...
const result = await tokagentOS.handleMessage(runtime, {
  entityId: userId,
  roomId: roomId,
...
```typescript
await tokagentOS.handleMessage(agentId, message, {
...
```typescript
const results = await tokagentOS.handleMessages([
  {
...
```typescript
// By ID
const agent = tokagentOS.getAgent(agentId);

// By name
const assistant = tokagentOS.getAgentByName("Assistant");

// All agents
const allAgents = tokagentOS.getAgents();

// Multiple by IDs
const selected = tokagentOS.getAgentsByIds([id1, id2]);

// Multiple by names
const team = tokagentOS.getAgentsByNames(["Researcher", "Writer"]);
```

### Events

TokagentOS extends `EventTarget` for lifecycle events:

```typescript
tokagentOS.addEventListener("agent:started", (e) => {
  console.log(`Agent ${e.detail.agentId} started`);
});

tokagentOS.addEventListener("message:sent", (e) => {
...
```typescript
// Check agent health
const health = await tokagentOS.healthCheck();
// Map<UUID, { alive: boolean, responsive: boolean, memoryUsage?: number, uptime?: number }>

// Validate API keys
const keyStatus = await tokagentOS.validateApiKeys();
// Map<UUID, boolean>
```
...
| Feature                  | Without TokagentOS                   | With TokagentOS                        |
| ------------------------ | --------------------------------- | ----------------------------------- |
...
The tokagentOS runtime follows a modular, plugin-based architecture that orchestrates all agent functionality. For lifecycle details, see [Runtime and Lifecycle](/agents/runtime-and-lifecycle). For extension architecture, see [Plugin Architecture](/plugins/architecture).
...
In `@tokagentos/core`, the embedding-generation service drains that queue through a **shared batch-queue pipeline** (priority queue, bounded concurrency, retries, repeat `EMBEDDING_DRAIN` task) so this path does not diverge from other queue-and-drain workloads. **Why:** one composable subsystem instead of multiple bespoke queue implementations as features grow. See [Batch queue subsystem](/runtime/batch-queue) (also `packages/typescript/docs/BATCH_QUEUE.md` in the monorepo).
...
### TokagentOS Type Guard

Check if the runtime has an TokagentOS instance attached:

```typescript
hasTokagentOS(): this is IAgentRuntime & { tokagentOS: ITokagentOS };
```

**Usage:**

```typescript
if (runtime.hasTokagentOS()) {
  // TypeScript knows runtime.tokagentOS is defined here
  const tokagentOS = runtime.tokagentOS;

  // Send message to another agent
  await tokagentOS.handleMessage(otherAgentId, {
