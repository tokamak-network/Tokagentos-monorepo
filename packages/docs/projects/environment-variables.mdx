...
description: Server configuration environment variables for TokagentOS
---

This document describes the server configuration environment variables for TokagentOS.

## Server Security & Authentication

### TOKAGENT_SERVER_AUTH_TOKEN

Controls API authentication for the TokagentOS server.

```bash .env
TOKAGENT_SERVER_AUTH_TOKEN=your-secret-token
```
...
```bash .env
NODE_ENV=production
TOKAGENT_SERVER_AUTH_TOKEN=secure-random-token-here
TOKAGENT_UI_ENABLE=false
```
...
# TOKAGENT_SERVER_AUTH_TOKEN=  # Unset for easy development
# TOKAGENT_UI_ENABLE=         # Unset for automatic behavior (UI enabled)
```

### Headless API Server

```bash .env
TOKAGENT_SERVER_AUTH_TOKEN=api-only-token
TOKAGENT_UI_ENABLE=false
```
...
```bash .env
NODE_ENV=production
TOKAGENT_SERVER_AUTH_TOKEN=my-api-key
TOKAGENT_UI_ENABLE=true
```
...
<Warning>
  **API Authentication**: In production, always set `TOKAGENT_SERVER_AUTH_TOKEN` to
...
2. **Recommended Production Setup**:
   - Set `TOKAGENT_SERVER_AUTH_TOKEN` to a strong, random value
   - Keep `TOKAGENT_UI_ENABLE=false` unless you need the web interface
...
- [`.env.example`](https://github.com/tokagentos/tokagent/blob/main/.env.example) in the repository - Template file showing all available environment variables with example values
...
<CardGroup cols={2}>
  <Card title="Deploy a Project" icon="rocket" href="/guides/deploy-a-project">
    Learn to deploy your TokagentOS project securely
  </Card>
