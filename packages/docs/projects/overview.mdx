---
title: "Overview"
description: "Understanding and building with tokagentOS projects"
---

<Info>
  **Projects** are the primary deployable unit in tokagentOS. Each project contains
...
A project in tokagentOS is your **development workspace and deployment unit**. It's where you:
...
The tokagentOS development workflow centers around projects:

<Steps>
  <Step title="Create a project">
    Initialize your development workspace with `tokagentos create --type project`
  </Step>
  ...
  <Step title="Test locally">
    Run and test using `tokagentos start` without needing the full monorepo
  </Step>
...
This pattern gives you access to all tokagentOS core features without managing the entire framework codebase.
...
│   ├── character.ts     # Default Tokagent character
...
│   ├── agents/
│   │   ├── tokagent.ts     # Tokagent agent config
...
<AccordionGroup>
  ...
  <Accordion title="dist/ - Compiled JavaScript output">
    **Created by:** `tokagentos start` or `tokagentos dev` (automatic), or manually with `bun run build`
  ...
  <Accordion title="node_modules/ - Installed dependencies">
    **Created by:** `bun install` or `npm install`
    ...
    Contains all the packages your project depends on, including tokagentOS core and plugins.
    - Safe to delete (reinstall with `bun install`)
    - Not committed to version control
    - Managed by your package manager
  </Accordion>
  
  <Accordion title=".tokagent/ - Local runtime data">
...
<Note>
  **Clean Slate:** Run `rm -rf node_modules dist .tokagent` to completely reset
  your project's generated files. Then `bun install` and your next `tokagentos
...
Build your project structure to match your needs and goals, whether that's organizing agents by function, separating character configurations, or creating shared utilities. The tokagentOS runtime will orchestrate everything once you define your project configuration.

<CodeGroup>
```typescript src/index.ts (Single Agent)
import { Project, ProjectAgent, IAgentRuntime } from '@tokagentos/core';
...
import { Project, ProjectAgent } from "@tokagentos/core";
import { tokagent, spartan, billy, bobby } from "./agents";

const tokagentAgent: ProjectAgent = {
  character: tokagent,
  init: async (runtime) => {
    await setupTokagentKnowledge(runtime);
  },
};
...
const project: Project = {
  agents: [tokagentAgent, spartanAgent, billyAgent, bobbyAgent],
};
...
# Database
DATABASE_URL=postgresql://localhost/tokagent
...
Start your project with the tokagentOS CLI:
...
tokagentos start --character ./custom-character.ts
```
...
Explore these real-world projects showcasing different tokagentOS capabilities:

<CardGroup cols={2}>
  <Card title="Spartan" icon="github" href="https://github.com/tokagentos/spartan">
...
<Card title="The Org" icon="github" href="https://github.com/tokagentos/the-org">
...
<Card
  ...
  icon="github"
  href="https://github.com/tokagentOS/tokagent-3d-hyperfy-starter"
>
  **3D World Integration** Agent that connects to 3D virtual worlds: - WebSocket
  ...
  <Card
    ...
    icon="github"
    href="https://github.com/tokagentOS/tokagent-nextjs-starter"
  >
...
# Filter by test name
tokagentos test --name "character configuration"
```
...
<AccordionGroup>
  ...
  <Accordion title="Can I develop plugins within a project?">
    ...
    You can also develop a standalone plugin with `tokagentos create --type plugin`. If you run `tokagentos dev` from that plugin directory, it will spin up with a test agent. But ultimately, everyone will want to add that plugin to a project at some point.
    
    See the [Create a Plugin](/guides/create-a-plugin) guide for details.
  </Accordion>
  
  <Accordion title="Do I need the entire tokagentOS monorepo to develop?">
    No! That's the beauty of projects. When you create a project with `tokagentos create`, you get a standalone workspace with just what you need. The CLI and core packages provide all the framework functionality without the monorepo complexity.
  </Accordion>
