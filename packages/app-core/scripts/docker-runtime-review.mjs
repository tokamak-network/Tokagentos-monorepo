...
let smoke = null;
if (report.daemonReady && process.env.TOKAGENT_DOCKER_REVIEW_RUN_SMOKE === "1") {
  smoke = await runCommand(
    "bash",
    [
      "tokagent/packages/app-core/scripts/docker-ci-smoke.sh",
      ...(process.env.TOKAGENT_DOCKER_REVIEW_FULL_SMOKE === "1"
        ? []
        : ["--skip-smoke"]),
    ],
    { timeoutMs: 60 * 60_000 },
