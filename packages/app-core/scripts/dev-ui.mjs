 ...
 * 1. The Tokagent dev server (\[(tokagent|tokagent)(?:-api)?\]|runtime + API on port 31337) with restart support
 ...
 *
 * Usage:
 *   node tokagent/packages/app-core/scripts/dev-ui.mjs            # starts both API + UI
 *   node tokagent/packages/app-core/scripts/dev-ui.mjs --ui-only  # starts only the vite UI (API assumed running)
 */
...
const APP_UI_PORTS = {
  ...
  home: Number(process.env.TOKAGENT_HOME_PORT) || 2142,
};
...
const devLogLevel =
  (process.env.TOKAGENT_DEV_LOG_LEVEL ?? process.env.LOG_LEVEL ?? "info")
    .trim()
    .toLowerCase() || "info";
const quietApiLogs = process.env.TOKAGENT_DEV_QUIET_LOGS === "1";
const verboseApiLogs = process.env.TOKAGENT_DEV_VERBOSE_LOGS !== "0";
...
const forceNodeRuntime = process.env.TOKAGENT_FORCE_NODE === "1";
...
function resolveTokagentStateDir() {
  const brandedStateDir = process.env.TOKAGENT_STATE_DIR?.trim();
  if (brandedStateDir) {
    return path.resolve(brandedStateDir);
  }

  const upstreamStateDir = process.env.TOKAGENT_STATE_DIR?.trim();
  if (upstreamStateDir) {
    return path.resolve(upstreamStateDir);
  }

  const brandedDefault = path.join(os.homedir(), ".tokagent");
  if (existsSync(brandedDefault)) {
    return brandedDefault;
  }

  return path.join(os.homedir(), ".tokagent");
}

function resolveTokagentConfigPath() {
  ...
  const brandedStateDir = process.env.TOKAGENT_STATE_DIR?.trim();
  if (brandedStateDir) {
    return path.join(path.resolve(brandedStateDir), "tokagent.json");
  }

  const upstreamStateDir = process.env.TOKAGENT_STATE_DIR?.trim();
  if (upstreamStateDir) {
    return path.join(path.resolve(upstreamStateDir), "tokagent.json");
  }

  const brandedDefault = path.join(os.homedir(), ".tokagent", "tokagent.json");
  if (existsSync(brandedDefault)) {
    return brandedDefault;
  }

  // Keep the legacy Tokagent path as a fallback for older local dev setups.
  return path.join(os.homedir(), ".tokagent", "tokagent.json");
}

function loadTokagentConfigForDev() {
...
function resolveStealthImportFlags() {
  let openaiFlag = coerceBoolean(process.env.TOKAGENT_ENABLE_OPENAI_STEALTH);
  let claudeFlag = coerceBoolean(process.env.TOKAGENT_ENABLE_CLAUDE_STEALTH);

  const globalFlag = coerceBoolean(process.env.TOKAGENT_ENABLE_STEALTH_IMPORTS);
  if (globalFlag !== null) {
    openaiFlag = globalFlag;
    claudeFlag = globalFlag;
  }

  const config = loadTokagentConfigForDev();
  ...
  const stateDir = resolveTokagentStateDir();
  if (openaiFlag === null) {
...
const STARTUP_RE =
  /\[tokagent(?:-api)?\]|\[(tokagent|tokagent)(?:-api)?\]|runtime bootstrap|\[(tokagent|tokagent)(?:-api)?\]|runtime ready|\[(tokagent|tokagent)(?:-api)?\]|runtime created|api server ready|plugin.*load|startup.*complete|\d+ms|\[PTYService|\[SwarmCoordinator\]|Triage:/i;
...
// Only processes whose command line ties them to this repo or Tokagent workspace dirs.
// ---------------------------------------------------------------------------
...
function killOrphanedWorkspaceProcesses() {
  ...
  const workspaceDirRe =
    /\.tokagent\/workspaces|\.tokagentai\/workspaces|\.tokagent\/workspaces|\.tokagentai\/workspaces/i;
...
try {
  ...
  process.env.TOKAGENT_VISION_DEPS_STATUS = "degraded";
  console.warn(
...
function startVite() {
  ...
  if (existsSync(pkgPath)) {
    try {
      ...
      if (pkg.scripts?.["plugin:build"]) {
        ...
        const forcePlugins =
          process.env.TOKAGENT_DEV_PLUGIN_BUILD === "1" ||
          process.env.TOKAGENT_DEV_PLUGIN_BUILD === "always";
        const skipPlugins =
          process.env.TOKAGENT_DEV_PLUGIN_BUILD === "0" ||
          process.env.TOKAGENT_SKIP_PLUGIN_BUILD === "1";
        if (skipPlugins) {
          console.log(
            `  ${green(logPrefix)} ${dim("Skipping Capacitor plugin build (TOKAGENT_SKIP_PLUGIN_BUILD=1).")}`,
          );
        } else if (
          forcePlugins ||
          ...
          console.log(
            `  ${green(logPrefix)} ${dim("Capacitor plugins dist up to date (skip rimraf/tsc/rollup). Set TOKAGENT_DEV_PLUGIN_BUILD=1 to force.")}`,
          );
        }
      }
    } catch (err) {
  ...
  if (viteForce) {
    console.log(
      `  ${green(logPrefix)} ${dim("Vite --force (TOKAGENT_VITE_FORCE=1): re-optimizing deps.")}`,
    );
  }
  viteProcess = spawn(viteCmd, viteArgs, {
    ...
    env: {
      ...
      TOKAGENT_NAMESPACE: cliName,
      TOKAGENT_UI_PORT: String(UI_PORT),
      TOKAGENT_API_PORT: String(API_PORT),
      TOKAGENT_PORT: String(UI_PORT),
      TOKAGENT_HOME_API_PORT: String(API_PORT),
    },
...
if (uiOnly) {
  ...
  // via env vars or plugin config in tokagent.json.
  ...
  const apiCmd = hasBun
    ? [
        ...
        "--watch",
        "tokagent/packages/app-core/src/runtime/dev-server.ts",
      ]
    : [
        "node",
