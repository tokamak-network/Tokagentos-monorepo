...
import {
  ...
  resolveTokagentAssetRepository,
  resolveTokagentReleaseTag,
...
function getValidationRetryPolicy({ env = process.env } = {}) {
  const explicitAttempts = Number.parseInt(
    env.TOKAGENT_CDN_VALIDATE_ATTEMPTS ?? "",
    10,
  );
  const explicitDelayMs = Number.parseInt(
    env.TOKAGENT_CDN_VALIDATE_DELAY_MS ?? "",
    10,
  );
  const explicitConcurrency = Number.parseInt(
    env.TOKAGENT_CDN_VALIDATE_CONCURRENCY ?? "",
    10,
  );
...
export function resolveValidationGitRef({
  ...
  return resolveTokagentReleaseTag({ env }) || resolveCurrentGitSha({ cwd, env });
}

export async function main({ cwd = repoRoot, env = process.env } = {}) {
  const releaseTag = resolveTokagentReleaseTag({ env });
  const gitSha = resolveCurrentGitSha({ cwd, env });
  const repository = resolveTokagentAssetRepository({ env });
  if (!releaseTag && !gitSha) {
    throw new Error(
      "Could not resolve release tag or git SHA for CDN validation. Set TOKAGENT_RELEASE_TAG / RELEASE_TAG or run inside a git checkout.",
    );
  }
