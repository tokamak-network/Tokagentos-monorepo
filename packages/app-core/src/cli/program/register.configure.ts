...
export function registerConfigureCommand(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/configuration", "docs.tokagent.ai/configuration")}\n`,
    )
    .action(() => {
      console.log(`\n${theme.heading("Configuration")}\n`);
      console.log("Set values with:");
      console.log(
        `  ${theme.command("tokagent config get <key>")}     Read a config value`,
      );
      console.log(`  Edit ~/.tokagent/tokagent.json directly for full control.\n`);
