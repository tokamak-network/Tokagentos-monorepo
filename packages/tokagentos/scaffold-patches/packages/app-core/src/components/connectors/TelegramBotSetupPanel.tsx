...
/**
 * Tokagent overlay vs upstream: the validate-token endpoint now mirrors
 * the token into process.env and schedules a runtime restart on save, so
 * "Connect" really means "connect" — the bot comes online without the
 * user manually restarting. UI surfaces a "Restarting agent..." state
 * for ~6 seconds after a successful save (the typical hot-restart window)
 * so the user knows the bot is coming up, not just saved-for-later.
 */
export function TelegramBotSetupPanel() {
    ...
    try {
      ...
      if (res.ok && res.bot) {
        ...
        if (res.restartScheduled) {
          // Server is restarting to load the plugin. Show a transient
          // restarting state so the user doesn't think nothing happened.
          // The agent process exits ~1s after the response; the dev-ui
          // host respawns it. Total restart cycle is typically <8s.
          setStatus("restarting");
          window.setTimeout(() => setStatus("connected"), 6000);
        } else {
          setStatus("connected");
        }
      } else {
  ...
  if (
    (status === "connected" || status === "restarting") &&
    botInfo
  ) {
    return (
      <PagePanel.Notice tone="accent" className="mt-4">
        <div className="space-y-2 text-xs">
          <div className="font-semibold text-txt">
            {status === "restarting"
              ? t("pluginsview.TelegramRestarting", {
                  defaultValue:
                    "Telegram bot saved — restarting agent to bring it online…",
                })
          ...
          {status === "connected" && (
            <div className="text-muted">
              {t("pluginsview.TelegramConnectedHint", {
                defaultValue:
                  "Your bot is live. Send it a message on Telegram to confirm.",
              })}
            </div>
          )}
          {status === "connected" && (
