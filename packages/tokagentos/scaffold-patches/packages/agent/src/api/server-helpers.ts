...
function buildWalletContextPrompt(
  ...
  // Tokagent overlay: broaden rpcReady beyond BSC-only — accept any
  // configured EVM RPC (EVM_PROVIDER_URL / ETHEREUM_PROVIDER_URL, populated
  // by the TOKAGENT_RPC_URL mirror in core-plugins.ts) so wallet questions
  // unblock for non-BSC users.
  const rpcReady = Boolean(
    process.env.BSC_RPC_URL?.trim() ||
      ...
      process.env.EVM_PROVIDER_URL?.trim() ||
      process.env.ETHEREUM_PROVIDER_URL?.trim(),
  );
