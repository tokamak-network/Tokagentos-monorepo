...
/**
 * Tokagent overlay: deterministic fallback for natural-language schedules.
 *
 * The LLM extraction frequently drops `intervalMs` / `cronExpression` when
 * the user says "every day", "daily", "every hour", etc. — leaving the
 * trigger validator to fail with `intervalMs is required for interval
 * triggers`. This walks the user text + extracted fields and fills in
 * concrete values when the LLM didn't, so common phrases produce a working
 * schedule instead of an error.
 *
 * Mutates `extracted` in place. Safe to call repeatedly. Only fills fields
 * that are still empty after LLM extraction — never overrides an explicit
 * value the LLM produced.
 */
function backfillNaturalLanguageSchedule(
  extracted: TriggerExtraction,
  userText: string,
): void {
  const text = userText.toLowerCase();
  const hasInterval = Boolean(extracted.intervalMs);
  const hasCron = Boolean(extracted.cronExpression);
  const hasOnce = Boolean(extracted.scheduledAtIso);
  const hasEvent = Boolean(extracted.eventKind);
  if (hasInterval || hasCron || hasOnce || hasEvent) return;

  const setCron = (expr: string): void => {
    extracted.cronExpression = expr;
    extracted.triggerType = "cron";
  };
  const setIntervalMs = (ms: number): void => {
    extracted.intervalMs = String(ms);
    extracted.triggerType = "interval";
  };

  // Common cron-style phrases — prefer cron over interval so DST + timezone
  // semantics stay correct.
  if (/\bevery\s+day\b|\bdaily\b|\beach day\b|\bonce a day\b/.test(text)) {
    setCron("0 9 * * *");
    return;
  }
  if (/\bevery\s+hour\b|\bhourly\b|\beach hour\b/.test(text)) {
    setCron("0 * * * *");
    return;
  }
  if (/\bevery\s+week\b|\bweekly\b|\beach week\b/.test(text)) {
    setCron("0 9 * * 1");
    return;
  }
  if (/\bevery\s+month\b|\bmonthly\b|\beach month\b/.test(text)) {
    setCron("0 9 1 * *");
    return;
  }

  // Numeric interval phrases — "every 5 minutes", "every 2 hours".
  const everyN = text.match(
    /\bevery\s+(\d+)\s*(second|minute|hour|day)s?\b/,
  );
...
export const createTriggerTaskAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      // Tokagent overlay: backfill schedule fields from natural-language
      // phrases ("every day", "hourly", "every 5 minutes"…) when LLM
      // extraction left them empty. Prevents the validator from rejecting
      // with "intervalMs is required for interval triggers".
      backfillNaturalLanguageSchedule(extraction, text);
