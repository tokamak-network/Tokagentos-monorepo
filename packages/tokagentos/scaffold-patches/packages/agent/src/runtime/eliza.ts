...
export function resolveWalletRuntimeSettings(
  ...
  // Tokagent overlay: forward EVM RPC env vars into runtime.settings so
  // plugin-evm's per-chain RPC resolver (rpc-providers.ts) finds them via
  // runtime.getSetting(). Without this, runtime.settings is empty for these
  // keys and plugin-evm falls back to viem's default mainnet RPC
  // (eth.merkle.io), which is currently slow and times out balance reads.
  //
  // The mirror in packages/agent/src/runtime/core-plugins.ts populates
  // process.env from TOKAGENT_RPC_URL; this block forwards those entries
  // through resolveWalletRuntimeSettings (which is spread into
  // runtime.settings without going through isEnvKeyAllowedForForwarding,
  // so the EVM_* prefix block at line ~1065 doesn't filter them out).
  for (const key of [
    "ETHEREUM_PROVIDER_MAINNET",
    "EVM_PROVIDER_MAINNET",
    "ETHEREUM_PROVIDER_URL",
    "EVM_PROVIDER_URL",
  ]) {
    const value = trimEnvString(env[key]);
    if (value) {
      settings[key] = value;
    }
  }
