...
/**
 * Tokagent overlay: background runtime tasks that the agent should never
 * see in its context (and never mention to the user). Mirrors the
 * SYSTEM_TASK_NAMES filter in app-core's automations-compat-routes.ts so
 * the chat agent agrees with the UI on what counts as a user-visible
 * task. Without this filter, asking the agent "what tasks do I have?"
 * surfaces internal embedding-drain / proactive-agent / heartbeat tasks.
 */
    ...
    async get(
      ...
      try {
        ...
        for (const task of allTasks) {
          ...
          // Tokagent overlay: skip system runtime tasks (EMBEDDING_DRAIN,
          // PROACTIVE_AGENT, LIFEOPS_SCHEDULER, TRIGGER_DISPATCH, heartbeat).
          // Otherwise the agent surfaces internal background tasks when the
          // user asks about their automations.
          if (SYSTEM_TASK_NAMES.has(task.name)) continue;
