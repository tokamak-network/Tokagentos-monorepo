 ...
 * runtime via `TOKAGENT_LIFEOPS_CONTEXT_WINDOW`.
 */
 ...
 *
 * Reads `TOKAGENT_LIFEOPS_CONTEXT_WINDOW` from the environment. Falls
 * back to {@link DEFAULT_CONTEXT_WINDOW} (16) when unset or invalid.
 */
export function resolveContextWindow(): number {
  const envValue = process.env.TOKAGENT_LIFEOPS_CONTEXT_WINDOW;
  if (envValue) {
