// `@tokagentos/plugin-telegram` was sourced from a github submodule that
// returns 404. Local fallbacks for the 3 exports app-lifeops uses — keeps
// the boot path alive at the cost of telegram-account features. When the
// plugin is restored, delete these stubs and re-import from the package.
// See: docs/eng-tickets/2026-05-16-tokagentos-boot-vs-plugin-sql-version-skew.md
function defaultTelegramAccountDeviceModel(): string {
  return "Tokagent";
}
function defaultTelegramAccountSystemVersion(): string {
  return "1.0";
}
function loadTelegramAccountSessionString(): string {
  // Without the upstream plugin we can't decrypt persisted sessions;
  // telegram account features stay disabled until the plugin is restored.
  return "";
}
